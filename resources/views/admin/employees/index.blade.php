        ...
        @cell('action', $employee)
            <div class="space-x-2">
                <Link href="{{ route('admin.employees.edit', $employee) }}"
                    class="text-green-400 hover:text-green-700 font-semibold"> View</Link>
                <Link href="{{ route('admin.employees.destroy', $employee) }}" method="DELETE"
                    class="text-red-400 hover:text-red-700 font-semibold" confirm="Delete the department"
                    confirm-button="Yes" cancel-button="No"> Delete</Link>
            </div>
        @endcell
    </x-splade-table>
</x-admin-layout>
